...
must_haves:
  truths:
    ...
    - "get_fsm_pending_files() in state.py gates on ai_metadata_status='approved' AND EXISTS(file_primary_topics) -- newly-scanned files with ai_metadata_status='pending' cannot reach fsm-upload"
  artifacts: []
  key_links:
...
**Step 6: Verify upload gate is enforced in get_fsm_pending_files().**
```bash
grep -A 20 "def get_fsm_pending_files" src/objlib/upload/state.py
```
Expected: The WHERE clause contains both `ai_metadata_status = 'approved'` AND `EXISTS (SELECT 1 FROM file_primary_topics pt WHERE pt.file_path = files.file_path)`.

Confirm a pending file is not returned by the gate:
```bash
sqlite3 data/library.db "SELECT COUNT(*) FROM files WHERE gemini_state = 'untracked' AND (filename LIKE '%.txt' OR filename LIKE '%.md') AND ai_metadata_status != 'approved'"
```
Expected: This count may be > 0 (files exist that are untracked + non-approved), but none of them would be returned by `get_fsm_pending_files()` because of the gate. The gate prevents them from entering the upload pipeline.
  </action>
  <verify>
...
9. `grep -A 20 "def get_fsm_pending_files" src/objlib/upload/state.py` shows `ai_metadata_status = 'approved'` AND `EXISTS (file_primary_topics)` in WHERE clause
  </verify>
  <done>
- `objlib metadata audit` exits 0 -- all 4 invariant conditions PASS
...
- get_fsm_pending_files() confirmed to have ai_metadata_status='approved' + EXISTS(file_primary_topics) gate (pending files cannot reach fsm-upload)
...
7. `grep -A 20 "def get_fsm_pending_files" src/objlib/upload/state.py` confirms upload gate: `ai_metadata_status = 'approved'` AND `EXISTS (SELECT 1 FROM file_primary_topics ...)` present in WHERE clause
</verification>
