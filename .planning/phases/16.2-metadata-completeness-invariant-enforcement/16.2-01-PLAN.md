...
Other-stem primary_topics coverage (files where scanner `metadata_json->>'topic'` equals the filename stem -- i.e., the topic was just derived from the filename, not a real semantic topic):
```sql
SELECT
  COUNT(*) as total,
  SUM(CASE WHEN EXISTS (SELECT 1 FROM file_primary_topics pt WHERE pt.file_path = f.file_path) THEN 1 ELSE 0 END) as with_topics
