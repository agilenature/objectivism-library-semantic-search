...
must_haves:
  truths:
    ...
    - "get_fsm_pending_files() in state.py gates on ai_metadata_status='approved' AND EXISTS(file_primary_topics) — a file with ai_metadata_status='pending' is NOT returned by this query"
  artifacts:
    ...
    - path: "src/objlib/upload/state.py"
      provides: "Upload gate: get_fsm_pending_files() requires approved status + file_primary_topics"
      contains: "file_primary_topics"
  key_links:
    ...
    - from: "_get_pending_files()"
      ...
      pattern: "LIKE '%.md'"
    - from: "get_fsm_pending_files()"
      to: "file_primary_topics junction table"
      via: "EXISTS subquery — upload gate precondition"
      pattern: "EXISTS.*file_primary_topics"
---

<objective>
Enforce the metadata completeness invariant across all 1,885 DB-tracked files by implementing the `mark-unsupported` and `audit` CLI commands, fixing the `.md` extension filter, backfilling 21 approved books missing primary_topics rows, detecting and resetting quality-failed files (Signal A: topic NULL + Signal B: all-boilerplate, excluding just-backfilled files), running batch-extract on all newly-pending enrichable files, and adding an upload gate to `get_fsm_pending_files()` that prevents files without approved AI metadata from reaching `fsm-upload`.

Purpose: Every file must either have `primary_topics` populated or `ai_metadata_status='skipped'` with a named reason. No file silently bypasses enrichment. The upload gate makes this invariant code-enforced as a precondition for Gemini upload — not just a manually-audited convention. This also produces the Phase 16.3 readiness data (MOTM topic field, MOTM primary_topics coverage, and Other-stem primary_topics coverage).

Output: Two new CLI commands (`metadata mark-unsupported`, `metadata audit`), one batch_orchestrator fix, one upload gate fix in `state.py`, backfilled junction rows, re-extracted MOTM metadata via Mistral Batch API, and audit reporting per-category breakdown with Phase 16.3 readiness.
</objective>
...
<task type="auto">
  <name>Task 3: Add upload gate to get_fsm_pending_files() — enforce metadata completeness as precondition for fsm-upload</name>
  <files>src/objlib/upload/state.py</files>
  <action>
IMPORTANT: Execute this task AFTER Tasks 1 and 2 are complete (after batch-extract has run and all enrichable files have `approved` status + `file_primary_topics` rows). Adding the gate before the retroactive fix would prevent the audit command from seeing files that still need processing.

**Step 1: Confirm the gap exists.**
```bash
grep -n "get_fsm_pending_files\|ai_metadata_status\|file_primary_topics" src/objlib/upload/state.py | head -30
```
Expected: `get_fsm_pending_files()` does NOT reference `ai_metadata_status` or `file_primary_topics`. If it already has these conditions, skip to Step 4.

**Step 2: Verify no indexed files would be blocked by the gate.**
Before touching the code, confirm that all currently-indexed enrichable files already have `file_primary_topics` rows (i.e., Tasks 1+2 succeeded):
```bash
sqlite3 data/library.db "SELECT COUNT(*) FROM files WHERE gemini_state='indexed' AND (filename LIKE '%.txt' OR filename LIKE '%.md') AND NOT EXISTS (SELECT 1 FROM file_primary_topics pt WHERE pt.file_path = files.file_path)"
```
Expected: 0. If non-zero, do not proceed — run `python -m objlib metadata approve` and re-check.

**Step 3: Add the upload gate to `get_fsm_pending_files()` in `src/objlib/upload/state.py`.**
Locate the SQL in `get_fsm_pending_files()` (around line 706–716). The current WHERE clause is:
```sql
...
```
This enforces the ordering invariant: `scan` → `metadata batch-extract` → `metadata approve` → `fsm-upload`. A newly-scanned file with `ai_metadata_status='pending'` will NOT be returned by this query and cannot be uploaded until it clears the metadata pipeline.

**Step 4: Also add the `file_primary_topics` gate to `get_files_to_reset_for_enriched_upload()` (around lines 449–461).**
That query currently has `ai_metadata_status IN ('extracted', 'approved', 'needs_review')`. Add:
```sql
AND EXISTS (SELECT 1 FROM file_primary_topics pt WHERE pt.file_path = f.file_path)
```
This ensures the re-upload path for already-indexed files also requires complete metadata before reset.
  </action>
  <verify>
1. `grep -A 20 "def get_fsm_pending_files" src/objlib/upload/state.py` — confirm WHERE clause includes `ai_metadata_status = 'approved'` and `EXISTS (SELECT 1 FROM file_primary_topics...)`
2. Simulate new-file scenario: `sqlite3 data/library.db "SELECT COUNT(*) FROM files WHERE gemini_state = 'untracked' AND (filename LIKE '%.txt' OR filename LIKE '%.md') AND ai_metadata_status = 'pending'"` — if any exist, confirm they would NOT be picked up by `fsm-upload` under the new gate
3. Confirm no existing indexed files are blocked: `sqlite3 data/library.db "SELECT COUNT(*) FROM files WHERE gemini_state = 'indexed' AND (filename LIKE '%.txt' OR filename LIKE '%.md') AND NOT EXISTS (SELECT 1 FROM file_primary_topics pt WHERE pt.file_path = files.file_path)"` — must return 0
  </verify>
  <done>
- `get_fsm_pending_files()` requires `ai_metadata_status='approved'` AND `EXISTS(file_primary_topics)` before a file is eligible for `fsm-upload`
- A newly-scanned file with `ai_metadata_status='pending'` cannot be uploaded until it clears the metadata pipeline
- `get_files_to_reset_for_enriched_upload()` also requires `EXISTS(file_primary_topics)` before resetting an indexed file
- No existing indexed files are blocked (all have topics after Tasks 1+2 retroactive fix)
- Metadata completeness is now code-enforced as a precondition for upload, not just a manually-audited convention
  </done>
</task>
