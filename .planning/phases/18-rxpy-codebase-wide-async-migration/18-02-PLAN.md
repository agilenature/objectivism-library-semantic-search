...
must_haves:
  truths:
    - "All asyncio.to_thread wrappers in services/search.py and services/library.py are replaced with RxPY observable wrappers using the Future-based subscription pattern"
...
- `asyncio.to_thread(fn, *args)` → RxPY observable with Future-based subscription (see below)
...
subscribed via the Future-based pattern within the function body to produce an awaitable
result.
...
<task type="auto">
  <name>Task 2: Migrate services/search.py to RxPY observable wrappers</name>
  <files>src/objlib/services/search.py</files>
  <action>
Read `src/objlib/services/search.py` in full before making any changes.

For each `asyncio.to_thread(fn, *args)` site, replace with the Future-based subscription
pattern. Do NOT use `.run()` — `Observable.run()` uses `threading.Event.wait()` which
blocks the calling thread. Inside an `async def`, this permanently blocks the asyncio
event loop, preventing any coroutines scheduled via `asyncio.ensure_future()` from
executing. The correct pattern for async contexts is:
...
async def _to_rx_async(fn, *args):
...
Replace each `await asyncio.to_thread(fn, arg1, arg2)` site with:
```python
result = await _to_rx_async(fn, arg1, arg2)
```

If the module has many to_thread calls, define `_to_rx_async` once at module level and
reuse it across all call sites.
...
2. `grep -n "rx.from_callable\|_to_rx_async\|result_future" src/objlib/services/search.py` → N results (one per old to_thread site)
3. `grep -n "\.run()" src/objlib/services/search.py` → 0 results (no .run() in async context)
4. `python -m pytest tests/ -k "search" -x -q` → passes
5. `python -c "from objlib.services.search import SearchService; print('OK')"` → OK
  </verify>
  <done>
services/search.py migrated. All to_thread calls replaced with Future-based subscription pattern. No .run() in async context. Tests pass.
  </done>
</task>

<task type="auto">
  <name>Task 3: Migrate services/library.py to RxPY observable wrappers</name>
  <files>src/objlib/services/library.py</files>
  <action>
Read `src/objlib/services/library.py` in full before making any changes.

Apply the same pattern as Task 2. Library has 6+ to_thread sites — use the `_to_rx_async`
helper pattern to avoid repetition. Define once at module level:
...
async def _to_rx_async(fn, *args):
    ...
    Uses Future-based subscription to bridge the RxPY observable into an awaitable.
    Do NOT use Observable.run() in async contexts — it blocks the event loop via
...
Pay attention to:
- Any to_thread calls that are inside comprehensions or loops (may need rx.from_iterable + ops.map + ops.merge)
- Any to_thread calls that pass results to other async functions
- Error handling: if the wrapped function raises, `on_error` sets the exception on the
  future, which propagates correctly to the caller via `await result_future`
...
2. `grep -n "\.run()" src/objlib/services/library.py` → 0 results (no .run() in async context)
3. `python -m pytest tests/ -k "library" -x -q` → passes
4. `python -c "from objlib.services.library import LibraryService; print('OK')"` → OK
  </verify>
  <done>
services/library.py migrated. All 6+ to_thread calls replaced with Future-based subscription pattern. Tests pass.
  </done>
</task>
...
**Note:** If the search client is sync (not async), `.run()` is acceptable here because
the call is not inside a running asyncio event loop — this is a synchronous method
invoked from a synchronous context. Check the module before deciding. If the method is
called from an async context, use the Future-based subscription pattern instead (same
as Task 2/3).
...
If to_thread calls are present in async methods, use the Future-based subscription
helper (`_to_rx_async`) — not `.run()`.
...
- Pattern used for each: _to_rx_async Future-based helper, retry_when variant, etc.
...
1. `grep -rn "asyncio.to_thread" src/objlib/services/search.py src/objlib/services/library.py src/objlib/search/client.py src/objlib/sync/orchestrator.py` → 0 results
2. `grep -rn "@retry" src/objlib/search/client.py` → 0 results (tenacity removed from this file)
3. `grep -rn "\.run()" src/objlib/services/search.py src/objlib/services/library.py src/objlib/sync/orchestrator.py` → 0 results (no .run() in async service contexts)
4. `python -m pytest tests/ -x -q` → ALL PASS (no regressions from Tier 3 migration)
5. `python -m objlib --store objectivism-library search "Rand"` → results returned without error
6. 18-02-SUMMARY.md documents all 4 files migrated with evidence
</verification>
...
- No Observable.run() calls in async service methods (Future-based subscription used instead)
