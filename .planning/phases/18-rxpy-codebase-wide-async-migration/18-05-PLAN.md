...
must_haves:
  truths:
    ...
    - "Canon.json updated to index all 10 migrated modules including _operators.py with all 4 operators (occ_transition, upload_with_retry, shutdown_gate, dynamic_semaphore)"
  ...
  artifacts:
    - path: "Canon.json"
      ...
      contains: "upload/orchestrator.py, upload/client.py, upload/state.py, upload/recovery.py, extraction/batch_orchestrator.py, extraction/orchestrator.py, services/search.py, services/library.py, search/client.py, sync/orchestrator.py, upload/_operators.py"
...
observable-based functions alongside (or replacing) coroutine-returning functions.
...
- `src/objlib/upload/_operators.py` — custom RxPY operators:
  - `occ_transition(fn, max_attempts, base_delay)` — OCC-guarded state transition (returns Observable)
  - `occ_transition_async(fn, max_attempts, base_delay)` — async wrapper, awaitable interface for state.py
  - `upload_with_retry(file_record, upload_fn, max_attempts)` — 429-aware upload retry
  - `shutdown_gate(obs, stop_accepting$, force_kill$=None)` — two-signal shutdown (Q4 contract)
  - `dynamic_semaphore(limit$: BehaviorSubject[int])` — BehaviorSubject-driven concurrency (Q2 contract)
...
3. `src/objlib/upload/_operators.py` is indexed with all 4 operators: occ_transition, occ_transition_async, upload_with_retry, shutdown_gate, dynamic_semaphore
4. `python -c "import json; json.load(open('Canon.json'))"` → no error
  </verify>
  <done>
Canon.json updated. All migrated modules indexed including _operators.py with all 4 operators. JSON valid.
  </done>
</task>
...
|---|---|---|---|
| services/search.py | 3 | to_thread → Future-based rx.from_callable | N |
| services/library.py | 3 | to_thread → Future-based rx.from_callable | N |
| search/client.py | 3 | @retry → retry_when | N |
| sync/orchestrator.py | 3 | light async → observable | N |
| extraction/batch_orchestrator.py | 2 | Semaphore/gather/polling → ops.map+ops.merge/interval | N |
| extraction/orchestrator.py | 2 | Semaphore/wave logic → ops.map+ops.merge/flat_map | N |
| upload/state.py | 1 | OCC retry → occ_transition_async | N |
| upload/client.py | 1 | AsyncRetrying → retry_when + Future-based subscription | N |
...
- `occ_transition(fn, max_attempts, base_delay)` — OCC-guarded state transition (Observable interface)
- `occ_transition_async(fn, max_attempts, base_delay)` — async wrapper for occ_transition; awaitable interface for state.py async methods; uses Future-based subscription (not .run())
- `upload_with_retry(file_record, upload_fn, max_attempts)` — 429-aware upload retry
- `shutdown_gate(obs, stop_accepting$, force_kill$=None)` — two-signal shutdown (Q4 contract): stop_accepting$ gates input, force_kill$ terminates active chains
- `dynamic_semaphore(limit$: BehaviorSubject[int])` — BehaviorSubject-driven concurrency control (Q2 contract): in-flight items complete on limit decrease, no cancellation
...
4. Custom Operators section lists all 4 operators: occ_transition, occ_transition_async, upload_with_retry, shutdown_gate (with two-signal Q4 signature), dynamic_semaphore
5. STATE.md updated to reflect Phase 18 complete
  </verify>
  <done>
...
7. Canon.json updated and valid JSON — includes _operators.py entry with occ_transition, occ_transition_async, upload_with_retry, shutdown_gate, dynamic_semaphore
8. 18-05-SUMMARY.md documents all verification steps with verbatim evidence and correct operator signatures
...
- Canon reflects the new reactive interface contracts including all 4 custom operators in _operators.py
