...
Note on schema design: `transcript_id` uses TEXT type to store file_path values, matching the existing database pattern where `files.file_path TEXT` is the PRIMARY KEY. There is NO separate `transcript` table with INTEGER ids in this codebase -- all file records live in the `files` table with TEXT file_path keys. The CLARIFICATIONS-ANSWERED.md Q3 example showing `INTEGER NOT NULL` was auto-generated without codebase knowledge and is incorrect; TEXT is the correct type for this codebase. Do NOT add formal FK to `files` table since the column name differs from `files.file_path` but semantically references it.
...
# Verify the EXACT set of blocked aliases (not just count)
blocked_rows = db.conn.execute("SELECT alias_text FROM person_alias WHERE is_blocked=1").fetchall()
blocked_set = {row[0].casefold() for row in blocked_rows}
expected_blocked = {'smith', 'aaron', 'tara', 'ben', 'mike', 'harry', 'greg', 'keith', 'don'}
assert blocked_set == expected_blocked, f'Blocked alias set mismatch. Missing: {expected_blocked - blocked_set}, Extra: {blocked_set - expected_blocked}'
```
...
Verify all 10 TDD test categories are present in the test file:
```bash
python -c "
import ast, sys

with open('tests/test_entity_extraction.py') as f:
...
# All 10 required test categories from the action section
REQUIRED_CATEGORIES = {
    'exact': 'exact match tests (Ayn Rand, Leonard Peikoff, etc.)',
    'alias': 'alias match tests (Peikoff, Rand, Dr. Peikoff, etc.)',
    'blocked': 'blocked alias tests (Smith, Aaron, Tara, etc.)',
    'disambiguation': 'disambiguation tests (Tara Smith vs Smith alone)',
    'possessive': 'possessive tests (Rand\'s, Peikoff\'s)',
    'speaker_label': 'speaker label tests (Leonard Peikoff: ...)',
    'full_transcript': 'full transcript extraction test (multi-paragraph)',
    'fuzzy': 'fuzzy match tests (Peikof typo, Binwanger typo)',
    'no_entities': 'no entities test (empty result)',
    'confidence': 'confidence threshold test (< 0.5 excluded)',
}

missing = []
for category, description in REQUIRED_CATEGORIES.items():
    if not any(category in name.lower() for name in test_names):
        missing.append(f'  - {category}: {description}')

if missing:
    print(f'FAIL: Missing test categories ({len(missing)}/{len(REQUIRED_CATEGORIES)}):')
    print('\n'.join(missing))
    print(f'\nFound tests: {test_names}')
    sys.exit(1)
else:
    print(f'All {len(REQUIRED_CATEGORIES)} TDD test categories present.')
    print(f'Total test functions: {len(test_names)}')
"
```
  </verify>
  <done>
