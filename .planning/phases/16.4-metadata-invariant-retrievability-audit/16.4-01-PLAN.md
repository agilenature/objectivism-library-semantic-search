...
<task type="auto">
  ...
  2. In `_mark_oversized()` (line ~493), add a comment referencing BOOK_SIZE_BYTES:
     The token-based MAX_DOCUMENT_TOKENS check remains (it catches files exceeding Mistral context window).
     Add a SECOND routing check in the batch request building loop (line ~152-164):
     BEFORE the token check, add a byte-size check:
     ```python
     # Book routing: files >= BOOK_SIZE_BYTES are books, skip extraction
     ...
     if file_size >= BOOK_SIZE_BYTES:
         ...
         self._mark_oversized(file_path, _estimate_token_count(
             Path(file_path).read_text(encoding="utf-8")
         ))
         continue
     ```
     IMPORTANT: Keep the existing token-based check AFTER this new byte check. They serve different purposes:
     - BOOK_SIZE_BYTES: structural routing ("this is a book, not a transcript")
     - MAX_DOCUMENT_TOKENS: Mistral context window limit (technical constraint)
  ...
  Record findings per file in SUMMARY. Any violation found must be fixed inline.
  </action>
  <verify>
  ...
  - `grep -rn "ai_metadata_status" src/objlib/ | wc -l` returns same count as pre-change (no new unaudited references)
...
3. `sqlite3 data/library.db "SELECT COUNT(*) FROM files WHERE ai_metadata_status='failed_validation'"` returns 0
4. `sqlite3 data/library.db "SELECT COUNT(*) FROM files WHERE ai_metadata_status='needs_review'"` returns 0
5. `sqlite3 data/library.db "SELECT COUNT(*) FROM files WHERE ai_metadata_status='pending' AND gemini_state='indexed'"` returns 41
6. Grep audit documents every ai_metadata_status reference across 6 source files
</verification>
