...
must_haves:
  truths:
    ...
    - "With empty store and 0 indexed files, all 6 assertions pass: count (0==0), DB->Store (trivial pass -- no indexed files), Store->DB (trivial pass -- empty store), stuck (0 uploading), search (N/A -- vacuous), citations (N/A -- vacuous)"
...
def _check_citation_resolution(self, citations: list | None, indexed_count: int) -> None:
    ...
    # Note: this intentionally opens a separate DB connection for isolation --
    # each assertion's DB access is independent, avoiding shared-state coupling.
