...
2. Verify old store still exists (dry run didn't delete):
  ...
  <done>
Store migration step is implemented with pre-flight check, 4-state recovery logic, create-before-delete ordering, force=True on deletion, and post-migration verification. Script ready for human execution.
  </done>
</task>
