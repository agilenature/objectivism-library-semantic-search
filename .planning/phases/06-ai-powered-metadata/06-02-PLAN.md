...
- `def select_test_files(db: Database, n: int = 20) -> list[dict]`:
  - Query the `files` table (this is the single source-of-truth table for all file records in the codebase -- there is no separate `transcript` table) for all TXT files with `category = 'unknown'` (using `json_extract(metadata_json, '$.category') = 'unknown'` AND `filename LIKE '%.txt'`)
...
# Verify sampler queries the correct 'files' table (not 'transcript')
python -c "
import inspect
from objlib.extraction.sampler import select_test_files
source = inspect.getsource(select_test_files)
assert 'files' in source.lower(), 'Sampler must query the files table'
assert 'transcript' not in source.lower(), 'No transcript table in codebase -- use files table'
print('Sampler table reference verified: queries files table')
"
```
  </verify>
  <done>
