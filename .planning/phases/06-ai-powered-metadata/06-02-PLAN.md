...
must_haves:
  ...
  artifacts:
    - path: "src/objlib/extraction/prompts.py"
      provides: "System prompts, user prompt template, and JSON schema injection for Mixtral"
...
- `class ExtractionOrchestrator`:
  ...
  - `async def run_wave1(self, test_files: list[dict], strategies: dict[str, StrategyConfig]) -> dict`:
    - For each file in test_files, for each strategy:
      ...
      - Override client temperature to strategy's temperature
      - Call `self._process_one(file_path, transcript, strategy)` with semaphore + rate limiter
  ...
  - `async def _process_one(self, file_path: str, transcript: str, strategy: StrategyConfig) -> dict`:
    ...
    - Time the API call
    - Call `self._client.extract_metadata(transcript_text=user_prompt, system_prompt=system_prompt, max_tokens=8000)`
