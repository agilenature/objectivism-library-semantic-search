...
must_haves:
  truths:
    ...
    - "MistralClient can make async API calls to magistral-medium-latest with JSON mode and parse thinking+text array responses"
  ...
  artifacts:
    ...
    - path: "src/objlib/extraction/client.py"
      provides: "MistralClient wrapper with async API calls, JSON mode, thinking block filtering"
...
- `CREATE TABLE IF NOT EXISTS file_metadata_ai` -- columns: metadata_id (PK AUTOINCREMENT), file_path (FK), metadata_json (TEXT NOT NULL), model (TEXT NOT NULL), model_version (TEXT), prompt_version (TEXT NOT NULL), extraction_config_hash (TEXT), is_current (BOOLEAN DEFAULT 1), created_at (TEXT DEFAULT strftime). Index on (file_path, is_current).
- `CREATE TABLE IF NOT EXISTS file_primary_topics` -- columns: file_path (FK), topic_tag (TEXT NOT NULL), PRIMARY KEY (file_path, topic_tag). Index on topic_tag.
- `CREATE TABLE IF NOT EXISTS wave1_results` -- columns: result_id (PK AUTOINCREMENT), file_path (FK), strategy (TEXT NOT NULL), metadata_json (TEXT NOT NULL), raw_response (TEXT), token_count (INTEGER), latency_ms (INTEGER), confidence_score (REAL), human_edit_distance (REAL), created_at (TEXT DEFAULT strftime).
...
```bash
# Verify imports work
python -c "from objlib.extraction.schemas import ExtractedMetadata, Category, Difficulty, CONTROLLED_VOCABULARY, MetadataStatus; print(f'Vocab size: {len(CONTROLLED_VOCABULARY)}'); assert len(CONTROLLED_VOCABULARY) == 40"
...
- `class MistralClient`:
  - `__init__(self, api_key: str, model: str = "magistral-medium-latest")` -- creates `Mistral(api_key=api_key)` client, stores model name.
  - `async def extract_metadata(self, transcript_text: str, system_prompt: str, max_tokens: int = 8000) -> tuple[dict, int]`:
    - Calls `await self._client.chat.complete_async(model=self._model, messages=[system, user], temperature=1.0, max_tokens=max_tokens, response_format={"type": "json_object"})`.
    - Calls `parse_magistral_response(response)` to get parsed dict.
    - Returns `(parsed_dict, response.usage.total_tokens)`.
    - Catches `MistralAPIException` -- re-raises `CreditExhaustedException` on 402, raises `RateLimitException` on 429.
  - Define `CreditExhaustedException(Exception)` and `RateLimitException(Exception)` in this module.
  - Import from `mistralai import Mistral` and from `mistralai.models import SDKError` (check actual exception class name in SDK -- use `SDKError` or `MistralAPIStatusException` with `.status_code`).
...
- `@config_app.command("remove-mistral-key")` -- deletes Mistral API key from keyring. Same pattern as existing `remove-api-key` command.
  ...
  <verify>
```bash
# Verify parser works with simulated responses
python -c "
from objlib.extraction.parser import parse_magistral_response
import json

# Test with a mock response object
class MockContent:
...
# Test array format (thinking + text)
resp = MockResponse([
    MockContent('thinking', 'Let me analyze...'),
    MockContent('text', json.dumps({'category': 'course_transcript', 'difficulty': 'intro'}))
])
result = parse_magistral_response(resp)
assert result['category'] == 'course_transcript'
print('Array parse OK')

# Test string format
resp2 = MockResponse(json.dumps({'category': 'qa_session'}))
