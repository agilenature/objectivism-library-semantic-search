...
- `class MistralClient`:
  ...
  - `async def extract_metadata(self, transcript_text: str, system_prompt: str, max_tokens: int = 8000, temperature: float = 1.0) -> tuple[dict, int]`:
    - Calls `await self._client.chat.complete_async(model=self._model, messages=[system, user], temperature=temperature, max_tokens=max_tokens, response_format={"type": "json_object"})`.
