...
must_haves:
  truths:
    - "CLI command 'objlib metadata extract-wave1' triggers competitive strategy execution on 20 test files"
    - "CLI command 'objlib metadata extract-wave1 --resume' resumes from checkpoint after credit exhaustion"
    - "CLI command 'objlib metadata wave1-report' generates side-by-side comparison of 3 strategies"
    - "Wave 1 report shows per-strategy metrics: avg tokens, avg latency, validation pass rate, confidence distribution"
    - "Quality gates evaluate whether any strategy meets Wave 2 transition criteria"
    - "Human can review results and select winning strategy or declare hybrid needed"
  artifacts:
