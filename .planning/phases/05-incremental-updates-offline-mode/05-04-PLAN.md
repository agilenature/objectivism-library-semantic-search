...
The upload command doesn't directly access the library path, but it uploads from the database which was populated by scan. No disk check needed for upload itself (it reads from DB and uploads to Gemini). Skip this -- upload only needs network, not disk.

Actually, re-check: the upload command reads file content from disk to upload to Gemini. So it DOES need disk access. Add a disk check at the start of the upload command function, similar to scan. The upload command doesn't have a --library parameter directly, but it reads file_path from the database which contains the library paths.

Add a disk guard to the upload command:
```python
