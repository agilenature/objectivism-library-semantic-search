...
must_haves:
  ...
  key_links:
    ...
    - from: "src/objlib/sync/orchestrator.py"
      ...
      via: "Composes EnrichedUploadOrchestrator for file upload"
      pattern: "EnrichedUploadOrchestrator|upload_and_import"
    - from: "src/objlib/sync/orchestrator.py"
...
Purpose: This is the primary deliverable of Phase 5 -- the sync command that enables incremental updates. It composes existing primitives (FileScanner for discovery, EnrichedUploadOrchestrator for upload, GeminiFileSearchClient for store operations) into a new sync workflow.
...
class SyncOrchestrator:
    ...
    Composes SyncDetector (change detection), EnrichedUploadOrchestrator
    (upload pipeline), and GeminiFileSearchClient (store operations).
...
- For the enriched upload path: You need to query file_metadata_ai and transcript_entity tables to build enriched metadata (same as EnrichedUploadOrchestrator._upload_enriched_file does). Extract this logic into a helper method `_build_file_upload_data(file_path, db)` that returns (custom_metadata, enriched_file_path_or_None, upload_hash).
- For AI metadata fallback (locked decision #3): If AI metadata lookup fails (no extraction exists), fall back to Phase 1 metadata via build_custom_metadata(). Log a warning but DO NOT skip the file.
