...
must_haves:
  truths:
    ...
    - "`objlib view <filename>` shows detailed view of a single document with full metadata panel"
    - "`objlib view <filename> --full` displays the complete source document text"
    - "`objlib view <filename> --show-related` queries Gemini for similar documents and displays them"
...
The `view` command is fully stateless (locked decision #4). Users copy a filename from `search` results and pass it directly -- no session state, no result indices, no re-querying.

Workflow: `objlib search "query"` shows compact results with filenames -> user copies filename -> `objlib view "filename.txt"` shows detailed view.

```python
@app.command()
def view(
    ctx: typer.Context,
    filename: Annotated[str, typer.Argument(help="Filename to view (e.g., 'Introduction to Objectivism.txt')")],
    full: Annotated[bool, typer.Option("--full", help="Show full document text in pager")] = False,
    show_related: Annotated[bool, typer.Option("--show-related", help="Show related documents via semantic similarity")] = False,
