...
while pgrep -f "python -m objlib enriched-upload" > /dev/null 2>&1; do
    ...
    FROM files 
    WHERE status IN ('uploaded', 'uploading', 'failed', 'pending')
    GROUP BY status 
    ORDER BY status;
    " | tee -a "$MONITOR_LOG"
...
sqlite3 data/library.db "SELECT status, COUNT(*) FROM files GROUP BY status ORDER BY status;" | tee -a "$MONITOR_LOG"
