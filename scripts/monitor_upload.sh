...
while kill -0 $PID 2>/dev/null; do
    ...
    if grep -i "error\|failed\|exception" "$LOG_FILE" > /dev/null 2>&1; then
        echo "⚠️  ERRORS DETECTED - Recent errors:" | tee -a "$MONITOR_LOG"
    ...
    sqlite3 data/library.db "SELECT status, COUNT(*) as count FROM files WHERE status IN ('uploaded', 'uploading', 'failed', 'pending') GROUP BY status ORDER BY status;" 2>/dev/null | tee -a "$MONITOR_LOG"
    echo "" | tee -a "$MONITOR_LOG"
